...
sudo -H -u duneuser bash -c "mkdir -p ~/.packages"
sudo -H -u duneuser bash -c "cd ~/.packages"
sudo -H -u duneuser bash -c "curl -L -O https://aur.archlinux.org/cgit/aur.git/snapshot/yay-bin.tar.gz"
sudo -H -u duneuser bash -c "tar -xvf yay-bin.tar.gz"
sudo -H -u duneuser bash -c "cd yay-bin && makepkg && mv *.zst .. && cd .. && rm -rf yay-bin"
sudo -H -u duneuser bash -c "sudo repo-add ./custom.db.tar.gz ./*"
